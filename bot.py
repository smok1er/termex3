...
keyboard = [
  ...
  [
    Button.url("المطور", "https://t.me/e_e_2")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ...
    legendbye = [
      [
        Button.url("دوس هنا", f"https://t.me/yhmmo")
        ]
      ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond("ارسل المشكله الى لحلها- @e_e_2\n**LOGS**\n" + str(e))
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("المالك", "https://t.me/e_e_2")
        ]
    ]
...
print(" لقد تم تنصيب البوت بنجاح.     للمساعده راسل المعرف التالي. @e_e_2")
